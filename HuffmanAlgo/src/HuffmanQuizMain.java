/**
 * HuffmanQuizMain.java This class is the starting point of this application. It
 * will demonstrates the compression and decompression of input string message
 * by command line argument or show results of unit testing if command line
 * argument has not provided.
 * 
...
public class HuffmanQuizMain {
	...
	/** Main function. it will executes when program starts. */
	public static void main(final String[] args) {
		...
		// It will run based on user input which user provided in command line
		// argument.
		if (inputString.length() > 0) {
			testHuffman(inputString, false);
		} else {
			System.out.println("   Unit testing of implemented program   ");
			System.out.println(" Provided different types of input to test   ");
			System.out.println("*****************************************");
	...
	/** This function will demonstrates compression and decompression part. */
		...
		try {
			...
			if (huffman.compress(input)) {
				...
				if (input.equals(outputString)) {
					final int percentage = (int) ((Math.abs(originalBytes
							- compressedBytes) / (double) originalBytes) * 100);
